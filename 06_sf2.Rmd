...
scaff_offset <- scaff_sizes %>% 
  mutate(offset = order_by(desc(scaffold_length),cumsum(scaffold_length))) %>% 
  mutate(scaff_type = as.integer(row_number() %% 2) )
...
nomi_sweep_genes <- read_table2("hpc/SF2/nomi_10_sweep_genes.gff",col_names = bedtools_cols,col_types = cols()) %>% 
  arrange(desc(sweep_score)) %>% 
  mutate(geneid = str_match(gene_attributes,pattern = "ID=([^;]+)")[,2]) %>% 
  left_join(scaff_offset, by=c("sweep_scaffold"="scaffold")) %>% 
  mutate(xpos = sweep_start + offset) %>% 
  rename(score=sweep_score) %>% 
  unite(col = 'sweep_id',sweep_scaffold,sweep_start,sep = "_")
```
...
tajima_stats <- read_tsv("hpc/angsd/north_Tajima_sweeps.tsv",col_names = c("scaffold","start","sweep_score","WinCenter","Tajima")) %>% 
  unite(col='sweep_id',scaffold,start,sep="_",remove = TRUE) %>% select(-WinCenter,-sweep_score) %>% 
  group_by(sweep_id) %>% 
