...
make_kraken_report(){
  ...
  kraken-report ${kraken_raw} > ${report_out}
  printf "Running kraken-mpa-report\n"
  kraken-mpa-report ${kraken_raw} > ${mpa_out}
}

export -f make_kraken_report

parallel -j 24 make_kraken_report /fast/shared/kraken_coral_symbionts {} ::: $(ls reads/*R1.fastq)
