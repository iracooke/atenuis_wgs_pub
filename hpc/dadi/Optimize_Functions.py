...
def Do_Optim_Rep(rep_round):
    reps_list,r,rep,fs,func,pts,params_perturbed,folds_list,upper_bound,lower_bound,maxiters_list,model_name,fs_folded = rep_round
    ...
    print "\t\t\tOptimized parameters fmin = ", params_opt

    params_opt_bfgs = dadi.Inference.optimize_log(params_opt, fs, func_exec, pts, lower_bound=lower_bound, upper_bound=upper_bound, verbose=1, maxiter=maxiters_list[r], output_file = "{}.log.txt".format(model_name))
    print "\t\t\tOptimized parameters bfgs = ", params_opt    

    #simulate the model with the optimized parameters
    sim_model = func_exec(params_opt_bfgs, fs.sample_sizes, pts)
    ...
    rep_results = collect_results(fs, sim_model, params_opt_bfgs, roundrep, fs_folded)
    
    return [rep,rep_results,roundrep,tb_rep]
...
def Optimize_Routine(fs, pts, outfile, model_name, func, rounds, param_number, fs_folded=True, reps=None, maxiters=None, folds=None, in_params=None, in_upper=None, in_lower=None, param_labels=" "):
    ...
    # All these are global variables so that we can reference them within pool workers
   # global fs,func,pts,best_params,folds,upper_bound,lower_bound,maxiters,model_name,results_list
...
#        import pdb;pdb.set_trace()
        ...
        worker_data = []
        for rep in range(reps_list[r]+1):
            ...
            worker_data.append([reps_list,r,rep,fs,func,pts,params_perturbed,folds_list,upper_bound,lower_bound,maxiters_list,model_name,fs_folded])


        pool = Pool(min(20,reps_list[r]))

        rep_outputs_list = pool.map(Do_Optim_Rep, worker_data)

        for rep,rep_results,roundrep,tb_rep in rep_outputs_list:
